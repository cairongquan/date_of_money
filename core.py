...
def getLoadingParams(value: str):
    ...
    for i in range(len(tempValueString)):
        ...
        if ~(rmbIndex is None) and (tempValueString[i] in numberList):
            if (
                len(numberIndexs) != 0
                and (numberIndexs[len(numberIndexs) - 1]["index"]) != i - 1
            ):
                lastIndex = None
                numberIndexs = []
            else:
                lastIndex = i
                numberIndexs.append({"index": i, "item": tempValueString[i]})
    ...
    for i in range(len(doc.ents)):
        ...
        if token.label_ == "MONEY":
            ...
            if (
                numberIndexs[len(numberIndexs) - 1]["item"]
                == list(result["price"])[len(list(result["price"])) - 1]
            ) and ~~(doc.ents[i - 1].label_ == "CARDINAL"):
                result["price"] = ""
